...
function normalize(s: string) {
  return s
    ...
    .replace(/[^a-z0-9]/g, " ")
    .replace(
      /\b(elementary|middle|high|school|senior|junior|academy|charter|magnet|center|k-8|k8|of|the|and|at|for|broward|county|home|pk|prek|excep|exceptional|gifted|technical)\b/g,
      " "
    )
    .replace(/\s+/g, " ")
    .trim();
...
async function fetchTitle(url: string): Promise<{ title: string; html: string } | null> {
  try {
    ...
    // Handle "Home - School Name" format
    let title = m ? m[1].trim() : "";
    if (title.toLowerCase().startsWith("home -") || title.toLowerCase().startsWith("home–")) {
...
async function main() {
  ...
  console.log(`Broward: ${schools.length} missing, probing ${SLUGS.length} URLs`);
  let found = 0;
  const BATCH = 12;
  for (let i = 0; i < SLUGS.length; i += BATCH) {
    ...
    process.stdout.write(
      `\r  ${Math.min(i + BATCH, SLUGS.length)}/${SLUGS.length} (${found} found)`
    );
  }
